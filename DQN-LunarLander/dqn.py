...
q_net = QNetwork(num_features=8).to(device)
target_net = QNetwork(num_features=8).to(device)
...
for episode in range(NUM_EPISODES):
    ...
    while not (done or trunc):
        ...
        next_state, reward, term, _, trunc = env.step(action)
        insert_transition((state, action, reward, next_state, term))
        state = next_state
        step_counter += 1
        ...
        # if episode_length >=200: trunc=True
        done = term or trunc
